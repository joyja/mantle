...
class Database {
  constructor(filename, desiredUserVersion) {
    this.desiredUserVersion = desiredUserVersion
    ...
    if (this.filename === `:memory:`) {
      this.client = new sqlite3.Database(`:memory:`, (error) => {
        if (error) {
          throw error
        }
      })
    } else {
      if (fs.existsSync(`${dir}/${this.filename}.db`)) {
        fileExisted = true
      }
      this.client = new sqlite3.cached.Database(
        `${dir}/${this.filename}.db`,
        (error) => {
          if (error) {
            throw error
          }
  ...
  async init() {
    await this.executeQuery('PRAGMA foreign_keys = ON', [], true)
    const { user_version: userVersion } = await this.executeQuery(
      'PRAGMA user_version',
      [],
      true
    )
  }
  async executeQuery(sql, params = [], firstRowOnly = false) {
    if (process.env.MANTLE_DEBUG) {
  ...
  setUserVersion(value) {
    if (
      ...
      this.fileExisted &&
      userVersion !== this.desiredUserVersion
    ) {
      fs.copyFileSync(
        `${dir}/${dbFilename}.db`,
    ...
    this.executeQuery(`PRAGMA user_version = ${desiredUserVersion}`, [], true)
  }
}
